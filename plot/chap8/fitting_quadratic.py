...
def set_ax(ax, data_type: str):
    ...
    labels = [f"{data_type}数据", "最佳二次拟合模型"]
    if data_type == "测试":
        labels.append("真实 h*")
    ax.legend(labels, loc="upper left", fontsize=12)


def plot_fitting_quadratic():
    ...
    p2 = Polynomial.fit(train["x"], train["y"], 2)
    ...
    _ = [ax.plot(xx, p2(xx), color="red", linewidth=1.0) for ax in axes]
    axes[1].plot(xx, p(xx), color="black", linewidth=1.0, linestyle="-")
