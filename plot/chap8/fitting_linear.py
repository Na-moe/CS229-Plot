...
def set_ax(ax, data_type: str):
    ...
    ax.legend([f"{data_type}数据", "最佳线性拟合模型"], loc="upper left", fontsize=12)


def plot_fitting_linear():
    ...
    p1 = Polynomial.fit(train["x"], train["y"], 1)
    ...
    _ = [ax.plot(xx, p1(xx), color="red", linewidth=1.0) for ax in axes]
